...
class ZipFileTest < MiniTest::Test
  ...
  def test_recover_permissions_after_add_files_to_archive
    ...
    zf = ::BimTools::Zip::File.new(src_zip, ::BimTools::Zip::File::CREATE)
    zf.add(entry_name, src_file)
    zf.close
    assert_equal(::File.stat(src_zip).mode, 0o100664)
  end
  ...
  def test_remove
    ...
    zf_read = ::BimTools::Zip::File.new(TEST_ZIP.zip_name)
  ...
  def test_rename_to_existing_entry
    old_entries = nil
    ::BimTools::Zip::File.open(TEST_ZIP.zip_name) { |zf| old_entries = zf.entries }
  ...
  def test_change_comment
    ...
    zf_read = ::BimTools::Zip::File.open(TEST_ZIP.zip_name)
